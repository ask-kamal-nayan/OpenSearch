...
public class RemoteSegmentStoreDirectoryFactory implements IndexStorePlugin.DirectoryFactory {
    ...
    /**
     * Create data formats for CompositeRemoteDirectory.
     * Uses PluginsService for discovery if available, otherwise fallback to defaults.
     */
    private Any createDataFormats() {
        if (pluginsService != null) {
            try {
                // Try to discover formats from plugins
