...
public class Store extends AbstractIndexShardComponent implements Closeable, RefCounted {
    ...
    public void cleanupAndVerifyComposite(String reason, MetadataSnapshot sourceMetadata, CompositeStoreDirectory compositeDirectory)
        ...
        metadataLock.writeLock().lock();
        try {
            // Clean up stale files from the Lucene store directory (segments_N lives here)
            for (String existingFile : directory.listAll()) {
                if (Store.isAutogenerated(existingFile) || sourceMetadata.contains(existingFile)) {
                    continue; // don't delete auto-generated files or files present in source
                }
                try {
                    directory.deleteFile(reason, existingFile);
                    logger.debug("Deleted stale Lucene file [{}] during {}", existingFile, reason);
                } catch (Exception ex) {
                    logger.debug(
                        () -> new ParameterizedMessage("failed to delete Lucene file [{}]", existingFile), ex);
                }
            }

            // Clean up stale files from CompositeStoreDirectory (format-specific files like parquet)
            ...
            // Build target metadata combining Lucene directory (segments_N) and CompositeStoreDirectory files
            final Map<String, StoreFileMetadata> combinedMetadata = new HashMap<>();

            // Include segments_N from the Lucene directory
            for (String existingFile : directory.listAll()) {
                if (existingFile.startsWith(IndexFileNames.SEGMENTS)) {
                    StoreFileMetadata sourceFileMeta = sourceMetadata.get(existingFile);
                    if (sourceFileMeta != null) {
                        combinedMetadata.put(existingFile, sourceFileMeta);
                    }
                }
            }

            // Include format-specific files from CompositeStoreDirectory
