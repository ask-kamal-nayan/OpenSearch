...
public class ParquetDocumentInputTests extends ParquetBaseTests {
    ...
    private MappedFieldType builderNumericFieldType(MappedFieldType.MultiValueState multiValue) {
        // The shared multi_value parameter is only wired onto the field type when a pluggable data
        // format is active, so the builder must be constructed from settings that opt in (paired with
        // @LockFeatureFlag on the callers). Otherwise applyMultiValueParameter is a no-op and the field
        // type reports scalar-only, which is not the behaviour under test.
        Settings settings = Settings.builder().put("index.pluggable.dataformat.enabled", true).build();
        NumberFieldMapper.Builder builder = new NumberFieldMapper.Builder("number", NumberFieldMapper.NumberType.INTEGER, settings);
        if (multiValue != MappedFieldType.MultiValueState.AUTO) {
            builder.setParameterValue("multi_value", multiValue);
        }
        NumberFieldMapper mapper = builder.build(new Mapper.BuilderContext(settings, new ContentPath()));
