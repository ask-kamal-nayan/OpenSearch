...
/// Reads the [`VALUES_SORTED_KEY`] marker from a Parquet file's key-value metadata.
/// Returns `true` ONLY when the key is present AND its value is exactly `"true"`.
/// Absent, empty, or any other value returns `false` — matching the read side's
/// default-safe behaviour (an unmarked file means the reader performs its own sort).
pub fn read_values_sorted(metadata: &FileMetaData) -> bool {
    metadata
        .key_value_metadata()
        .and_then(|kvs| {
            kvs.iter()
                .find(|kv| kv.key == VALUES_SORTED_KEY)
